...
public class Map {
    ...
    public static VertexArrayObject vao =
            new VertexArrayObject(EntityManager.vertices, EntityManager.indices);
    public static int vaoID = vao.getVaoID();

    public static final String[] maps = {"fractal", "soft", "stand", "trig", "quad", "e2", "e"};
    public static final String[] splots = {"grit", "exp", "ln", "rng", "arm", "disk"};// there are
                                                                                      // more
...
//        generateFoliage();
        ...
        for (int chunkX = 0; chunkX < chunks.length; chunkX++) {
            for (int chunkY = 0; chunkY < chunks[0].length; chunkY++) {
                for (int i = 0; i < Chunk.CHUNKSIZE * Chunk.CHUNKSIZE * 2; i++) {
//                    for (int z = 0; z < Map.WORLDHEIGHT; z++) {
                        System.out.println(i);
                        int a = (i / Chunk.CHUNKSIZE) % Chunk.CHUNKSIZE + chunkX;
                        int b = i / (Chunk.CHUNKSIZE * Chunk.CHUNKSIZE);
                        int h = i % Chunk.CHUNKSIZE + chunkY;
//                        a = 0;
//                        h = 15;
                        data[i] = land[a][b][h];
                    }
...
//                }
     ...
     * 
     * int[] land = new int[HEXESACROSS * HEXESDOWN]; int counter = 0; for (int x = 0; x <
     * HEXESDOWN; x++) { for (int y = 0; y < HEXESACROSS; y++) { land[counter] = Map.land[y][x];
     * counter++; } } int bufferID = glGenBuffers();
     * 
     * glBindBuffer(GL_ARRAY_BUFFER, bufferID); glBindBuffer(GL_TEXTURE_BUFFER, bufferID); IntBuffer
     * data = Utilities.createIntBuffer(land); glBufferData(GL_ARRAY_BUFFER, data, GL_STATIC_DRAW);
     * 
     * int textureID = glGenTextures(); glBindTexture(GL_TEXTURE_BUFFER, textureID);
     * glTexBuffer(GL_TEXTURE_BUFFER, GL_R32UI, bufferID); glBindBuffer(GL_ARRAY_BUFFER, 0);
     * 
     * glActiveTexture(GL_TEXTURE5); glBindTexture(GL_TEXTURE_BUFFER, textureID);
     * 
     * }
     */

    /*
     * @Deprecated private Hexagon[] getAllNeighbors(Hexagon hex) { if (hex.xIndex > 0 && hex.yIndex
     * > 0 && hex.xIndex < HEXESACROSS - 1 && hex.yIndex < HEXESDOWN - 1) { Hexagon[] neighbors =
     * new Hexagon[6]; neighbors[0] = hexes.get(hex.yIndex + 1).get(hex.xIndex); neighbors[1] =
     * hexes.get(hex.yIndex - 1).get(hex.xIndex); neighbors[2] =
     * hexes.get(hex.yIndex).get(hex.xIndex + 1); neighbors[3] =
     * hexes.get(hex.yIndex).get(hex.xIndex - 1); if (hex.xIndex % 2 == 0) { neighbors[4] =
     * hexes.get(hex.yIndex - 1).get(hex.xIndex + 1); neighbors[5] = hexes.get(hex.yIndex -
     * 1).get(hex.xIndex - 1); } else { neighbors[4] = hexes.get(hex.yIndex + 1).get(hex.xIndex +
     * 1); neighbors[5] = hexes.get(hex.yIndex + 1).get(hex.xIndex - 1); } return neighbors; } else
    ...
    public void zoom(float zoomFactor) {
        ...
        ShaderManager.chunkShader.setUniform3f("pos",
                new Vector3f(-zoomFactor + offsetX, zoomFactor + offsetY, 0));
    ...
    /*private double getP(String genType, double p, int iter) {
        ...
        switch (genType) {
            ...
            case "invT":
                ...
                System.exit(-1);
                p = ((Math.pow(Math.cos(iter) * (1.9), 2)) - (1 * Math.abs(rng.nextDouble()))
                        + 0.1);
                break;
            case "e":
    ...
    /*private void initializeMap() {
        ...
        /*
         * int[][] neighbors = getNeighborIndices(seeds[0][0], seeds[0][1]); for (int j = 0; j <
         * neighbors.length; j++) { if (land[neighbors[j][0]][neighbors[j][1]] != LAND &&
         * land[neighbors[j][0]][neighbors[j][1]] != SEED) { land[neighbors[j][0]][neighbors[j][1]]
         * = LAND; } }
        ...
        for (int i = 0; i < HEXESACROSS; i++) {
            for (int j = 0; j < HEXESDOWN; j++) {
                ...
                /*
                 * if (land[i][j] == SEED) { elevation[i][j] = 4; moisture[i][j] = 4; for (int k =
                 * 0; k < neighbors.length; k++) { elevation[neighbors[k][0]][neighbors[k][1]] = 20;
                 * moisture[neighbors[k][0]][neighbors[k][1]] = 20; } }
                 
                if (land[i][j] == LAND) {

                    int elev = (int) (Math.abs(
                            noise.getValue(i / ELEVATIONSCALER, j / ELEVATIONSCALER, .1)) * 16);
                    int moist = (int) (Math
                    ...
                    // elev = (elev / 5 + (int) (4+4*rng.nextDouble()));//one day this will all be
                    // perlin
        ...
        /*
         * for (int i = 0; i < HEXESACROSS; i++) { for (int j = 0; j < HEXESDOWN; j++) { if
         * (land[i][j] == LAND) { land[i][j] = Block.getBlock(elevation[i][j], worldType,
         * moisture[i][j]); } } }
