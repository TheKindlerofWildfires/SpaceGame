...
public class Chunk {
	...
	private static final float[] bottom = { 2 / sqrt3, 0, 0, 0, 1, 1, 0, 0, -1,// right
																				// 0
			1 / sqrt3, -1, 0, 1, 0, 1, 0, 0, -1,// lower right 1
			-1 / sqrt3, -1, 0, 1, 1, 0, 0, 0, -1,// lower left 2
			-2 / sqrt3, 0, 0, 1, 0, 1, 0, 0, -1,// left 3
	...
	private static final float[] front = { -1 / sqrt3, 1, 1, 1, 0, 1, 0, 1, 0,// upper
																				// left
																				// 4
			1 / sqrt3, 1, 1, 1, 0, 1, 0, 1, 0,// upper right 5
			1 / sqrt3, 1, 0, 1, 0, 1, 0, 1, 0,// upper right 5
	...
	private static final float[] back = { 1 / sqrt3, -1, 1, 0, 1, 0, 0, -1, 0,// lower
																				// right
																				// 1
			-1 / sqrt3, -1, 1, 0, 1, 0, 0, -1, 0,// lower left 2
			-1 / sqrt3, -1, 0, 0, 1, 0, 0, -1, 0,// lower left 2
	...
	private static final float[] frontRight = { 1 / sqrt3, 1, 1, 1, 1, 0,
			1 / 2, sqrt3 / 2, 0,// upper right 5
			2 / sqrt3, 0, 1, 1, 1, 0, 1 / 2, sqrt3 / 2, 0,// right 0
			2 / sqrt3, 0, 0, 1, 1, 0, 1 / 2, sqrt3 / 2, 0,// right 0
			1 / sqrt3, 1, 0, 1, 1, 0, 1 / 2, sqrt3 / 2, 0,// upper right 5
	};

	private static final float[] frontLeft = { -2 / sqrt3, 0, 1, 0, 1, 1,
			-1 / 2, sqrt3 / 2, 0,// left 3
			-1 / sqrt3, +1, 1, 0, 1, 1, -1 / 2, sqrt3 / 2, 0,// lower left 2
			-1 / sqrt3, +1, 0, 0, 1, 1, -1 / 2, sqrt3 / 2, 0,// lower left 2
			-2 / sqrt3, 0, 0, 0, 1, 1, -1 / 2, sqrt3 / 2, 0 // left 3
	};

	private static final float[] backLeft = { -1 / sqrt3, -1, 1, 0, 0, 1,
			-1 / 2, -sqrt3 / 2, 0,// lower left 2
			-2 / sqrt3, 0, 1, 0, 0, 1, -1 / 2, -sqrt3 / 2, 0,// left 3
			-2 / sqrt3, 0, 0, 0, 0, 1, -1 / 2, -sqrt3 / 2, 0,// left 3
			-1 / sqrt3, -1, 0, 0, 0, 1, -1 / 2, -sqrt3 / 2, 0,// lower left 2
	};

	private static final float[] backRight = { 2 / sqrt3, 0, 1, 1, 0, 0, 1 / 2,
			-sqrt3 / 2, 0,// right 0
			1 / sqrt3, -1, 1, 1, 0, 0, 1 / 2, -sqrt3 / 2, 0,// lower right 1
			1 / sqrt3, -1, 0, 1, 0, 0, 1 / 2, -sqrt3 / 2, 0,// lower right 1
			2 / sqrt3, 0, 0, 1, 0, 0, 1 / 2, -sqrt3 / 2, 0,// right 0
	...
	public static byte[] indices = new byte[] { 0, 1, 2, 0, 2, 3, 0, 3, 4, 0,
			4, 5, 6, 7, 8, 6, 8, 9, 6, 9, 10, 6, 10, 11, 12, 13, 14, 12, 14,
			15, 16, 17, 18, 16, 18, 19, 20, 21, 22, 20, 22, 23, 24, 25, 26, 24,
			26, 27, 28, 29, 30, 28, 30, 31, 32, 33, 34, 32, 34, 35 };

	public static byte[] hexesIndices = { 0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 5, };

	public static byte[] sidesIndices = { 0, 1, 2, 0, 2, 3 };
	// @formatter:on
	...
	private static final VertexArrayObject fullVAO = new VertexArrayObject(
			vertices, indices);
	...
	public Chunk(int[][][] properties, int chunkX, int chunkY) {
		...
		model = Matrix4f.translate(1.5f * 1.2f * chunkX * CHUNKSIZE - 10,
		...
		// HOW BIG ARE MY ARRAYS??? NO ONE KNOWS (Except this part of the code.
		// it knows)
		...
		UBO = Utilities.createUniformBuffer(Utilities
				.createUniformFloatBuffer(Utilities.flatten(properties)));
	}
	...
	public void render() {
		...
		for (int i = 0; i < normals.length; i++) {
			if (normals[i].dot(cameraDir) > 0) { // MAKES IT FASTER, BUT NEEDS
													// TWEAKING
				ShaderManager.chunkShader.setUniform3f("normal", normals[i]);
				glBindVertexArray(VAOS[i].getVaoID());
				glDrawElementsInstanced(GL_TRIANGLES,
						(i == BOTTOM || i == TOP) ? hexesIndices.length
								: sidesIndices.length, GL_UNSIGNED_BYTE, 0,
