...
public class Chunk {
	...
	private VertexArrayObject topVAO;
	private VertexArrayObject bottomVAO;
	private VertexArrayObject frontVAO;
	private VertexArrayObject backVAO;
	private VertexArrayObject frontLeftVAO;
	private VertexArrayObject frontRightVAO;
	private VertexArrayObject backLeftVAO;
	private VertexArrayObject backRightVAO;

	private Matrix4f model;
	private Matrix4f normal;

	private int numberOfTopPts;
	private int numberOfBottomPts;
	private int numberOfFrontPts;
	private int numberOfBackPts;
	private int numberOfFrontLeftPts;
	private int numberOfFrontRightPts;
	private int numberOfBackLeftPts;
	private int numberOfBackRightPts;
	...
	public Chunk(float[][][] properties, int chunkX, int chunkY) {
		...
		topVAO = new VertexArrayObject(top, 2);
		...
		bottomVAO = new VertexArrayObject(bottom, 2);

		numberOfBottomPts = bottom.length / 2;
		...
		frontVAO = new VertexArrayObject(front, 2);

		numberOfFrontPts = front.length / 2;
		...
		backVAO = new VertexArrayObject(back, 2);

		numberOfBackPts = back.length / 2;
		...
		frontRightVAO = new VertexArrayObject(frontRight, 2);

		numberOfFrontRightPts = frontRight.length / 2;
		...
		frontLeftVAO = new VertexArrayObject(frontLeft, 2);

		numberOfFrontLeftPts = frontLeft.length / 2;
		...
		backLeftVAO = new VertexArrayObject(backLeft, 2);

		numberOfBackLeftPts = backLeft.length / 2;
		...
		backRightVAO = new VertexArrayObject(backRight, 2);

		numberOfBackRightPts = backRight.length / 2;
	}
	...
	public void render() {
		...
		glBindVertexArray(topVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfTopPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(0, 0, -1));
		glBindVertexArray(bottomVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfBottomPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(0, 1, 0));
		glBindVertexArray(frontVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfFrontPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(0, -1, 0));
		glBindVertexArray(backVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfBackPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(1 / 2, sqrt3 / 2, 0));
		glBindVertexArray(frontRightVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfFrontRightPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(-1 / 2, sqrt3 / 2, 0));
		glBindVertexArray(frontLeftVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfFrontLeftPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(1 / 2, -sqrt3 / 2, 0));
		glBindVertexArray(backRightVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfBackRightPts);

		ShaderManager.chunkShader.setUniform3f("normal", new Vector3f(-1 / 2, -sqrt3 / 2, 0));
		glBindVertexArray(backLeftVAO.getVaoID());
		glDrawArrays(GL_TRIANGLES, 0, numberOfBackLeftPts);
