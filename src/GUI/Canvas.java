 ...
 * 
 * @author Simon
 * paintComponent calls twice--screen refresh
 */
...
public class Canvas extends JPanel {
		...
		*/
		...
		for (int i = 0; i < HEXESACROSS; i++) {
			for (int j = 0; j < HEXESDOWN; j++) {
				if (j % 2 == 0) {
					hexes.get(j).add(new Hexagon(i * 2 * apothem, (int) (j * 3 * apothem / Math.sqrt(3))));
				} else {
					hexes.get(j).add(new Hexagon(i * 2 * apothem + apothem, (int) (j * 3 * apothem / Math.sqrt(3))));
				}
			}
		}
		for (int i = 0; i < HEXESACROSS; i++) {
	...
	public void paintComponent(Graphics g) {
		...
		Hexagon seed = hexes.get(HEXESACROSS/4+rng.nextInt(HEXESACROSS/2)).get(HEXESDOWN/4+rng.nextInt(HEXESDOWN/2));
		Hexagon seed2 = hexes.get(HEXESACROSS/4+rng.nextInt(HEXESACROSS/2)).get(HEXESDOWN/4+rng.nextInt(HEXESDOWN/2));
		Hexagon seed3 = hexes.get(HEXESACROSS/4+rng.nextInt(HEXESACROSS/2)).get(HEXESDOWN/4+rng.nextInt(HEXESDOWN/2));
		seed.setLand(true);
		if (seedCount >= 2){
			seed2.setLand(true);
		}
		...
		while (outerLand.size() != 0) {
			...
			if (mapType == "sfractal"){
				...
				i = Math.cos(1.43*i);
			/*}else if (mapType == "disk"){
				i = Math.pow(2.6, -2.46*i)/(0.01*(seedCount-1)+1);
				*/
			}else{
				i = 0;
			}
